defmodule Test4it210.ConfusionMatrix do
  ...
  defmodule ConfusionMatrixQuestion do
    ...
    @enforce_keys [:results, :confusion_matrix, :precision_class, :recall_class, :precision, :recall, :accuracy]
    defstruct [:results, :confusion_matrix, :precision_class, :recall_class, :precision, :recall, :accuracy]
  end
  ...
  defp compute_precision(%ConfusionMatrix{true_positive: true_positive, false_positive: false_positive}, true),
    do: true_positive / (true_positive + false_positive)

  defp compute_precision(%ConfusionMatrix{true_negative: true_negative, false_negative: false_negative}, false),
    do: true_negative / (true_negative + false_negative)

  @spec compute_recall(ConfusionMatrix.t(), boolean()) :: float()
  defp compute_recall(%ConfusionMatrix{true_positive: true_positive, false_negative: false_negative}, true),
    do: true_positive / (true_positive + false_negative)

  defp compute_recall(%ConfusionMatrix{true_negative: true_negative, false_positive: false_positive}, false),
  ...
  defp compute_accuracy(matrix) do
    (matrix.true_positive + matrix.true_negative) / (matrix.true_positive + matrix.true_negative + matrix.false_positive + matrix.false_negative)
  end
  ...
  defp generate_results() do
    Enum.map(1..10, fn index -> %Result {id: index, prediction: random_boolean(), reality: random_boolean() } end)
  end
